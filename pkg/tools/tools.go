...
// SizeFormat Converts byte number to correct unit (up to gigabyte).
...
// SizeFormatComplete Converts byte number to correct unit.
func SizeFormatComplete(size int, prc ...int) string {
	digitNumbers := 2
	if len(prc) > 0 {
		if prc[0] > 0 {
			digitNumbers = prc[0]
